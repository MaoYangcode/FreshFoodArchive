...
const NORMALIZED_NAME_TO_ICONFONT = Object.keys(NAME_TO_ICONFONT).reduce((acc, key) => {
	...
	if (normalized && !acc[normalized]) acc[normalized] = NAME_TO_ICONFONT[key]
	return acc
}, {})
...
const NAME_MATCH_ITEMS = Object.keys(NAME_TO_ICONFONT)
	.map((key) => ({
...
export function getIngredientIconfontClass(name) {
	...
	if (normalizedHit) return normalizedHit.icon
