...
function isTinyRecipeUnit(unitText) {
  const text = `${unitText || ""}`.trim();
  if (!text)
    return false;
  return /(勺|匙|茶匙|汤匙|小勺|大勺|少许|适量|撮|滴)/.test(text);
}
function isLikelyCondiment(item) {
  const category = `${(item == null ? void 0 : item.category) || ""}`.trim();
  if (category === "调味品")
    return true;
...
function pickSmartPurchaseSpec(item) {
  const name = `${(item == null ? void 0 : item.name) || ""}`.trim();
...
function toSmartBasketItem(rawItem) {
  const item = {
    name: `${(rawItem == null ? void 0 : rawItem.name) || ""}`.trim(),
    quantity: Number((rawItem == null ? void 0 : rawItem.quantity) || 1),
    unit: `${(rawItem == null ? void 0 : rawItem.unit) || "份"}`.trim() || "份",
    category: `${(rawItem == null ? void 0 : rawItem.category) || ""}`.trim() || "其他"
  };
  if (!isLikelyCondiment(item) || !isTinyRecipeUnit(item.unit))
    return item;
...
exports.toSmartBasketItem = toSmartBasketItem;
//# sourceMappingURL=../../.sourcemap/mp-weixin/utils/smart-purchase.js.map
