...
function getCategoryFallbackIconfontClass(category) {
  ...
  if (cat === "水果")
    return "icon-shuiguolanzi";
  if (cat === "蔬菜")
    return "icon-huacai1";
  if (cat === "海鲜")
    return "icon-shengxian-haixian1";
  if (cat === "肉类")
    return "icon-shengxian-roulei";
  if (cat === "蛋奶")
    return "icon-niunai";
  if (cat === "饮料")
    return "icon-yinliao";
  if (cat === "调味品")
    return "icon-tiaoliao";
  if (cat === "其他")
    return "icon-bianzu";
  return "";
}
function getIngredientDisplayIconfontClass(name, category = "") {
  return getIngredientIconfontClass(name) || getCategoryFallbackIconfontClass(category);
}
function getIngredientWeappColorClass(name, category = "") {
